...
public enum EncodingMode
	...
	public Bitmap CreateQRCodeBitmap
			...
			)
		{
			...
			// encode data
			EncodeQRcode(new byte[][] { BinaryData });
			return;
		}
				...
				)
		{
			...
			// encode data
			EncodeQRcode(new byte[][] { SingleDataSeg });
			return;
		}
				...
				)
		{
            ...
            for (int SegIndex = 0; SegIndex < DataSegArray.Length; SegIndex++)
            {
				Bytes = DataSegArray[SegIndex].Length - 4;
				if (Bytes % 3 == 1) throw new ArgumentException("There is no data to encode.");
				byte[] base45Buf = b45decode(DataSegArray[SegIndex]);

				byte[] cose = ZlibStream.UncompressBuffer(base45Buf);

                byte[] decrypted = Com.AugustCellars.COSE.Message.DecodeFromBytes(cose).GetContent();

				CBORObject cbor = CBORObject.DecodeFromBytes(decrypted);

				string jsonDecoded = cbor.ToJSONString();

				MessageBox.Show(jsonDecoded);
			}			
        }
		...
		public byte[] b45decode
			(
				byte[] Databuf
			)
        {
			int Bytes = Databuf.Length - 4;
			if (Bytes % 3 == 1) throw new ArgumentException("There is no data to encode.");

			byte[] base45Buf = new byte[Bytes];
            for (int Index = 0; Index < Bytes; Index++)
            {
                int Code = StaticTables.EncodingTable[(int)Databuf[Index + 4]];
                base45Buf[Index] = (byte)Code;
            }

			byte[] res = null;
			int x;
			for(int Index = 0; Index< Bytes; Index+=3)
            {
				if(Bytes - Index>=3)
                {
					x = base45Buf[Index] + base45Buf[Index + 1] * 45 + base45Buf[Index + 2] * 45 * 45;

					if(res==null)
						System.Array.Resize(ref res, 2);
					else
						System.Array.Resize(ref res, res.Length + 2);

					res[res.Length - 2] = (byte)(x / 256);
					res[res.Length - 1] = (byte)(x % 256);				
				}
                else
                {
					x = base45Buf[Index] + base45Buf[Index + 1] * 45;
					System.Array.Resize(ref res, res.Length + 1);
					res[res.Length - 1] = (byte)(x);
				}
            }
